...
- 筛选订阅节点（将订阅节点筛选成用户需要的代理节点，减少过多代理节点出现在策略组内，方便用户选择需要的代理节点）
  ...
  - [筛选类型为 `NameRegex` 的筛选方法](https://github.com/TiyNa/LoonManual/blob/main/Plus/Remote_Filter.md)（利用正则表达式筛选订阅节点）
...
- 设置 SSID策略组
  ...
  - [方法一：内置 ssid 策略组的设置](https://github.com/TiyNa/LoonManual/blob/main/Plus/ssid.md)（此教程需要理解较多概念，而且较为单一，不够灵活）
  
  - [进阶操作：ssid 策略组的嵌套](https://github.com/TiyNa/LoonManual/blob/main/Plus/ssid_Plus.md)（操作极其繁琐，但是满足需求，只是较难理解，新手、无耐心用户请绕道）（未完成）
